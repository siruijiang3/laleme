...
达到容量保护线时保留已经写入的数据，不自动升级套餐。后续实际完成范围和数量以
生产数据库的 `osm_sync_runs` 成功记录为准。

首次生产导入已于 **2026-09-15 01:37（香港时间）**完成：53/53 区域成功，新增
**79,455** 个去重后的美国厕所点位，另有 319 次跨区域重复对象更新；跳过和删除均为 0。
[首次同步运行记录](https://github.com/siruijiang3/laleme/actions/runs/34871421463)。
原有中国区域点位和用户贡献保留；纽约、洛杉矶、芝加哥、阿拉斯加、夏威夷及深圳的
地图范围查询与公开 API 抽查通过。该数量是首次导入快照，后续每周同步会继续变化。
